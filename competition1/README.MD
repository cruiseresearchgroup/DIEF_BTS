...
3. Download [`random_submission_py.py`](/competition1/random_submission_py.py)
4. Create a directory called `data` in the same location as the `random_submission_py.py` file, and put the dataset in the `data/` directory. 
5. Extract filenames from a ZIP file containing test data (.pkl files).
6. Run [`random_submission_py.py`](/competition1/random_submission_py.py) locally, and it will generate a sample submission file `data/sample_submission_v0.1.0.csv.gz`. Might take few minutes.
